...
class Event extends Controller
{
    ...
    public function chat()
    {
        ...
        $banword = Db::table('wolive_banword')->where(['business_id'=>$arr['business_id'],'lang'=>$lang,'status'=>1])->whereLike('keyword',"%{$arr['content']}%")->find();
        ...
        try {
            ...
            $set = new Set();
            $arr['content_trans'] = $set->isTrans($arr['content'],'zh',$arr['business_id']);
            $arr['direction'] = 'to_service';
            $chatModel = new Chats();
            if($chatModel->allowField(true)->save($arr)){
...
//        $ip = $_SERVER["REMOTE_ADDR"];
    ...
    public function uploadLocation(){
        $post = $_POST;
        $ak = Db::name('wolive_business')->where(['id'=>$post['business_id']])->value('bd_location_ak');
        if(empty($ak)) return ['code' => 1, 'msg' => $this->lang_array['seek_failed']];
        $query = http_build_query(["ak" => $ak,"output" => 'json',"language" => 'local',"location" => $post['location'],]);
        try{
            $res = file_get_contents("https://api.map.baidu.com/reverse_geocoding/v3/?$query");
            $res = json_decode($res,true);
            if($res['status'] == 0){
                $service = Admins::table('wolive_queue')->where('business_id', $post['business_id'])->where('visiter_id', $post['visiter_id'])->where('state', 'normal')->find();
                if ($service['service_id'] != $post['service_id']) return ['code' => 1, 'msg' => $this->lang_array['session_close']];
                $sarr = parse_url(ahost);
                ...
                $post["timestamp"] = time();
                $post['service_id'] = $service['service_id'];
                $post['content'] = $html;
                $pusher->trigger('kefu' . $service['service_id'], 'cu-event', array('message' => $post));
                $post['direction'] = 'to_service';
                $chatModel = new Chats();
                $chatModel->allowField(true)->save($post);
                return $data = ["code" => 0,"msg" => "","data" => $res['result']['formatted_address']];
            }else{
                return ['code' => 1, 'msg' => $this->lang_array['seek_failed']];
            }
        }catch (Exception $e) {
            return ['code' => 1, 'msg' => $this->lang_array['seek_failed']];
        }
    ...
    public function getanswer()
    {
        ...
        if ($result) {
            ...
            $arr['avatar'] = $service['avatar']?:'/assets/images/index/ai_service.png';
            $arr['tip'] = '';
