...
class Set extends Base
{
    ...
    {
        $login = $_SESSION['Msg'];
        if(strpos($_POST['content'], '.wav') !== false){
            $arr = $_POST;
        }else{
    ...
    public function uploadLocation(){
        $login = $_SESSION['Msg'];
        $arr = $this->request->post();
        $ak = Db::name('wolive_business')->where(['id'=>$login['business_id']])->value('bd_location_ak');
        if(empty($ak)) return ['code' => 1, 'msg' => '定位失败！请填写百度地图API AK'];
        $query = http_build_query(["ak" => $ak,"output" => 'json',"language" => 'local',"location" => $arr['location'],]);
        try{
            $res = file_get_contents("https://api.map.baidu.com/reverse_geocoding/v3/?$query");
            $res = json_decode($res,true);
            if($res['status'] == 0){
                $html = '<a href="http://api.map.baidu.com/geocoder?location='.$res['result']['location']['lat'].','.$res['result']['location']['lng'].'&output=html&src=webapp.baidu.openAPIdemo" style="display: block;" target="_blank"><p style="margin-bottom: 10px">'.$res['result']['formatted_address'].'</p><img src="/assets/images/admin/B/map.png"/></a>';
                $arr['content'] = $html;
                $arr['business_id'] = $login['business_id'];
                $arr['service_id'] = $login['service_id'];
                $arr['direction'] = 'to_visiter';
                ...
                $channel = bin2hex($arr['visiter_id'] . '/' . $arr['business_id']);
                $sarr = parse_url(ahost);
                $state = ($sarr['scheme'] == 'https')?true:false;
                $app_key = app_key;
                $app_secret = app_secret;
                $app_id = app_id;
                ...
                //针对同一客户端的锁，防止同一客户端多次回调
                $_SESSION[$key] = md5(microtime(true));
                return $data = ["code" => 0,"msg" => "","data" => $res['result']['formatted_address']];
            }else{
                return ['code' => 1, 'msg' => '定位失败'];
            }
        }catch (Exception $e) {
            return ['code' => 1, 'msg' => '定位失败'];
        }
     ...
     * @return bool
     */
	private function safeCheck($text, $lang)
    {
        if ($lang != 'cn') return false;
        $word = ['转账', '充值', '现金', '提现', '银行卡', '支付', '返利', '投资', '理财', '金融'];
        foreach ($word as $v) if (strpos($text, $v) !== false) return true;
        return false;
    }
    ...
    public function isTrans($text,$to,$business_id){
        ...
        if(strpos($text, '<audio') !== false) return $text;
...
//wolive_chats 消息表
        ...
        foreach ($data as $v) {
            ...
            if ($values) {
                ...
                if (strpos($chats2['content'], 'api.map.baidu.com') !== false)  $newcontent = '[定位]';

            } else {

                if (strpos($chats2['content'], '</audio>') !== false) {
                    $newcontent = '[语音]';
                } elseif (strpos($chats2['content'], '</i>') !== false ) {
                    $newcontent = '[文件]';
...
//D:\web\kfsystem\public\assets\css\index\ai_service.css
    ...
    public function createMinDiyjs()
    {
        ...
        if($type == 2){
            ...
            $div = "blzxMinChatWindowDivs";
            $mdiv = "minblzxmsgtitlecontainerlabels";
            $hdiv = "hidden";
        }
                     ...
                     document.getElementById('blzxMinChatWindowDiv').style.display=\"none\";
                      ...
                      var moblieweb = encodeURI('{$url}{$this->base_root}/mobile/index/home?{$theme}visiter_id='+blzx.visiter_id+'&visiter_name='+blzx.visiter_name+'&avatar='+blzx.avatar+'&business_id={$business_id}&groupid='+groupid+'&product='+blzx.product);
                       ...
                       if(!s){
                            ...
                            document.body.appendChild(div);
                            div.innerHTML='<i class=\"blzx-close\" onclick=\"blzx.closeMinChatWindow(\'wolive-talk\')\"></i><iframe id=\"wolive-iframe\" src=\"'+web+'\" onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
